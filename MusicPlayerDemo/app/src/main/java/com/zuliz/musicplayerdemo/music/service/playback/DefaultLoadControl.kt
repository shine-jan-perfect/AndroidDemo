...
class DefaultLoadControl(
    ...
    class Builder {
        ...
        fun createDefaultMyControl(): DefaultLoadControl {
            return build()
        }
    ...
    @Deprecated("Use {@link DefaultMyControl.Builder} instead. ")
    ...
    override fun shouldContinueLoading(
        ...
        minBufferUs = Math.max(minBufferUs, Int.MAX_VALUE.toLong())
        if (bufferedDurationUs < minBufferUs) {
            isBuffering = prioritizeTimeOverSizeThresholds || !targetBufferSizeReached
            if (!isBuffering && bufferedDurationUs < Int.MAX_VALUE) {
                Log.w(
                    "DefaultMyControl",
